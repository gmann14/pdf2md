#!/usr/bin/env npx tsx
/**
 * Blind Quality Evaluation Set 3 — Stress Tests
 * Tests edge cases: very long docs, dense footnotes, timetables, legal briefs, etc.
 *
 * Usage: npx tsx --require ./test-corpus/node-polyfills.cjs test-corpus/evaluate-blind-3.ts
 */
...
function getCategory(filename: string): string {
  const categories: Record<string, string> = {
    "stress-gpo-style-manual-2016": "Long Document (475pp)",
    "stress-scotus-loper-bright": "Dense Footnotes (Legal)",
    "stress-llm-survey-bibliography": "Bibliography-Heavy Survey",
    "stress-njtransit-northeast-corridor": "Schedule/Timetable",
    "stress-icc-amicus-brief": "Legal Brief (Numbered)",
    "stress-neurips-2023-paper": "Conference Proceedings",
    "stress-irs-pub17-tax-guide": "FAQ/Knowledge Base",
  };
...
// ---------------------------------------------------------------------------
// Scoring functions (identical to training set)
// ---------------------------------------------------------------------------
...
function scoreParagraphIntegrity(md: string, category: string): number {
  ...
  const shortBlocksExpected = category === "Schedule/Timetable";
...
function scoreTableDetection(md: string, category: string): number {
  ...
  const expectTables = ["Schedule/Timetable"];
...
async function main(): Promise<void> {
  ...
  const corpusDir = resolve(scriptDir, "blind-pdfs-3");
  const resultsDir = resolve(scriptDir, "blind-results-3");

  await mkdir(resultsDir, { recursive: true });

  const files = (await readdir(corpusDir)).filter((f) => f.endsWith(".pdf")).sort();

  console.log(`\n🔬 BLIND QUALITY TEST SET 3 — ${files.length} stress test PDFs\n`);
  console.log("Edge cases: long docs, dense footnotes, timetables, legal briefs.\n");
  ...
  console.log(`\nBLIND TEST SET 3 (Stress): ${avgScore}/10 average (${successful.length} PDFs)`);
  ...
  // Save report
  const reportPath = resolve(scriptDir, "BLIND-TEST-3-REPORT.md");

  const reportLines: string[] = [];
  reportLines.push("# Blind Quality Test Report — Set 3 (Stress Tests)");
  reportLines.push("");
  reportLines.push(`**Generated:** ${new Date().toISOString().split("T")[0]}`);
  reportLines.push(`**PDFs Tested:** ${evaluations.length} (stress tests)`);
  reportLines.push(`**Set 3 Average Score:** ${avgScore}/10`);
  ...
  reportLines.push("");
  reportLines.push("| Dimension | Set 3 Avg | Training Avg | Blind Set 1 Avg |");
  reportLines.push("| --------- | --------: | -----------: | --------------: |");
