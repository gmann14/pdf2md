#!/usr/bin/env npx tsx
/**
 * Blind Quality Evaluation — runs unseen PDFs through @pdf2md/core
 * to validate quality on documents the converter was never optimized against.
 *
 * Usage: npx tsx --require ./test-corpus/node-polyfills.cjs test-corpus/evaluate-blind.ts
 */
...
// ---------------------------------------------------------------------------
// Category mapping for blind test PDFs
// ---------------------------------------------------------------------------

function getCategory(filename: string): string {
  const categories: Record<string, string> = {
    "medical-cdc-sti-report": "Medical/Healthcare Report",
    "patent-waymo-autonomous": "Patent",
    "textbook-mit-calculus-ch15": "Textbook (Math)",
    "annual-report-amazon-2023": "Annual Report",
    "who-tb-report-2024": "International Org Report",
    "spec-rfc9293-tcp": "Technical Specification",
    "thesis-mit-ir-2024": "Thesis/Dissertation",
    "brochure-nps-grand-canyon": "Brochure",
    "taxform-uk-sa100": "Government Form (Non-US)",
    "poster-exanest-hipeac": "Research Poster",
    "cookbook-tsu-recipes": "Cookbook/Recipe",
  };
...
// ---------------------------------------------------------------------------
// Scoring functions (identical to evaluate.ts — no category-specific tuning)
// These are the SAME scorers from the training set, applied unchanged.
// ---------------------------------------------------------------------------
...
function scoreHeadingDetection(md: string, category: string): number {
  ...
  if (
    ...
    category === "Government Form (Non-US)"
  ) {
...
function scoreListDetection(md: string, category: string): number {
  ...
  // Categories where lists may not exist
  const listsOptional = [
    "Patent",
    "Brochure",
    "Research Poster",
    "Government Form (Non-US)",
    "Textbook (Math)",
    "Annual Report",
  ];
...
function scoreCodeBlockDetection(md: string, category: string): number {
  ...
  // Categories where code blocks are expected
  const expectCode = ["Technical Specification"];
...
// ---------------------------------------------------------------------------
// Evaluation and reporting (reused from evaluate.ts)
// ---------------------------------------------------------------------------
...
async function main(): Promise<void> {
  ...
  const corpusDir = resolve(scriptDir, "blind-pdfs");
  const resultsDir = resolve(scriptDir, "blind-results");

  await mkdir(resultsDir, { recursive: true });

  const files = (await readdir(corpusDir)).filter((f) => f.endsWith(".pdf")).sort();

  console.log(`\n🔬 BLIND QUALITY TEST — ${files.length} unseen PDFs\n`);
  ...
  console.log(`\nBLIND TEST RESULTS: ${avgScore}/10 average (${successful.length} PDFs)`);
  ...
  // Save results
  const reportPath = resolve(scriptDir, "BLIND-TEST-REPORT.md");

  const reportLines: string[] = [];
  reportLines.push("# Blind Quality Test Report");
  reportLines.push("");
  ...
  reportLines.push(`**Blind Average Score:** ${avgScore}/10`);
  ...
  reportLines.push("");
  reportLines.push("## Dimension Averages (Blind vs Training)");
  reportLines.push("");
  reportLines.push("| Dimension | Blind Avg | Training Avg |");
  reportLines.push("| --------- | --------: | -----------: |");
  ...
  for (const key of scoreKeys) {
    const vals = successful.map((e) => e.scores[key]);
    const avg = vals.length > 0 ? Math.round((vals.reduce((s, v) => s + v, 0) / vals.length) * 10) / 10 : 0;
    reportLines.push(`| ${key} | ${avg}/10 | ${trainingAvgs[key] ?? "?"}/10 |`);
  }
